...
def treinar_modelo_logistico(hist):
    """IA que aprende com todo o histórico usando Regressão Logística real"""
    if len(hist) < 300:
        return 50.0, 50.0  # fallback
    df = hist.copy()
    # Target: alta nos próximos 15 dias
...
def processar_bitcoin():
    ...
    # ===================== IA REGRESSÃO LOGÍSTICA (APRENDE COM HISTÓRICO) =====================
    prob_alta, acuracia_ia = treinar_modelo_logistico(hist)
    # ===================== VEREDITO INTELIGENTE =====================
    if prob_alta > 75:
        veredito, cor = "COMPRA FORTE 🟢", "success"
        motivo_detalhe = f"Probabilidade real de alta em 15 dias: {prob_alta}% (IA treinada em 5 anos de dados)"
    elif prob_alta > 60:
...
# ===================== PROCESSAMENTO =====================
with st.spinner("📡 Baixando dados do Bitcoin e treinando IA com Regressão Logística..."):
...
with tab1:
    if dados_vencedoras:
        ...
        # ALERTA INTELIGENTE TIPO BLOOMBERG
